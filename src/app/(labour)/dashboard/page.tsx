...
import { User, Briefcase, DollarSign, MapPin, CheckCircle2, Clock, XCircle } from "lucide-react";
import { updateFundiRates } from "@/features/labour/actions/update-profile";

// Note: In production, get the user ID from the session (e.g., getServerSession)
// For this robust UI implementation, we are fetching the first available profile as a placeholder.
export default async function LabourDashboardPage() {
  let profile = null;
  let applications = [];

  try {
    profile = await prisma.labourProfile.findFirst({ include: { user: true, applications: { include: { job: true } } } });
    if (profile) {
          ...
          <div className="lg:col-span-1 space-y-6">
            <Card>
              <CardContent className="p-6 text-center">
                ...
                <h2 className="text-xl font-bold text-dark">{profile.user.name}</h2>
                <p className="text-primary font-semibold">{profile.skill.replace('_', ' ')}</p>
                <div className="mt-4 space-y-2 text-sm text-gray-600 text-left">
                  <div className="flex items-center gap-2"><MapPin className="h-4 w-4" /> {profile.county}</div>
                  <div className="flex items-center gap-2"><Briefcase className="h-4 w-4" /> {profile.yearsExperience} Years Exp.</div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <form action={updateFundiRates} className="space-y-4">
                  ...
                  <div>
                    ...
                    <Input name="dailyRate" type="number" defaultValue={profile.dailyRate} required />
                  </div>
                  <div>
                    <label className="text-xs font-medium text-gray-500">Monthly Rate (KES)</label>
                    <Input name="monthlyRate" type="number" defaultValue={profile.monthlyRate || ""} />
                  </div>
                  ...
                  <div className="space-y-4">
                    {applications.map((app: any) => (
                      <div key={app.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 border border-grey-dark rounded-lg hover:bg-grey/50 transition-colors">
                        <div>
                          <h3 className="font-bold text-dark">{app.job.title}</h3>
                          <p className="text-sm text-gray-500">{app.job.location} • Applied {new Date(app.appliedAt).toLocaleDateString()}</p>
                          <p className="text-sm text-primary font-semibold mt-1">Expected: KES {app.expectedRate}/day</p>
                        </div>
